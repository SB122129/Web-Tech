...
JavaScript (JS) is a high-level, interpreted programming language that is one of the three core technologies of the World Wide Web, along with HTML and CSS. It allows web developers to create interactive web pages.

# What are Variables?

Variables are containers for storing data (storing data values).

Variables are used to store data values. They are one of the most basic and fundamental concepts in programming. In JavaScript, variables are declared using the `let` keyword. The value of a variable can be changed or updated at any point in the program. Here is an example:
...
They are also declared using the `var` keyword. For example:
